...
datos Segment
	...
	errorFormato db 'Este formato no está permitido$'
...
agregaranio:
	...
	jmp leercomandos
;*************************************************************************
;Manipula el año para que sea negativo en caso de que se detecte el signo*
;*************************************************************************
ManipularA:
...
;*********************************************************
;Define si es un año completo, un solo mes o un único día*
;*********************************************************
confirmarModo:
	modoGrafico
	...
	cmp y,-5777;confirma que el año no sea menor a -5777
	jl noPermitido
	cmp y,7777;confirma que el año no sea mayor a 7777
	jg noPermitido
	...
	cmp m,12;confirma que el mes esté dentro del rango aceptable
	jg noPermitido
...
saltoContinuar2a1:
	mov ax,ndias
	cmp q,ax;confirma que sea un día valido para el mes
	jg noPermitido
	restaurarValores
	jmp continuar1b
...
imprimirMes:
	...
	jmp comprobarNumeroDias
;*********************************************
;Comprueba el número de días que tiene un mes*
;*********************************************
comprobarNumeroDias:
...
zeller:
	...
	add cx, 12;de lo contrario se modifica el año y mes
	mov [m], cx
	sub dx, 1
	mov [y], dx
...
verif2:
	cmp [q], 15;confirma si es gregoriano del año 1582
	jge verif3
...
ciclodiferencia:			;se calculan los días de error correspondientes al calendario juliano para así realizar su debido desplazamiento
	...
	cmp y2,1501;dependiendo del año, la diferencia es disntinta
	jge intervalo20
	cmp y2,1501
	jl diferencia
	jmp modifanio
...
crearLineasHorizontales:
	...
	mov ax, limite2
	cmp plineas, ax;compara el limite vertical especificado con la posición actual de la linea a imprimir
	je setLimite2
	...
	add plineas,16;guarda el valor de 'y' de la siguiente línea
...
finalizar:
	...
	mov ax, y2;restaura el año
	mov y, ax
	
	mov q, 1;vuelve al primer día del mes
	cmp cantidadMeses, 12;confirma que se hayan imprimido 12 años
	jne repetir
...
;**************************************************************************
;calcula la semana, el numero de día y los restantes para finalizar el año*
;**************************************************************************
continuar:
...
;***************************************************
;calcula el dia de la semana del primer día del mes*
;***************************************************
continuar1b:
	mov ax,d;respalda 'd'
	mov d2,ax
	...
	jmp zeller
;*********************************************************
;guarda el día obtenido y restaura los valores originales*
;*********************************************************
continuar1c:
	...
	mov ubi,ax
	mov ax,d ;guarda el día conseguido en d2
	mov d2,ax
...
fin11a:
	...
	add ax,4;añade la diferencia generada por el desplazamiento de días del 1 de enero
	xor dx, dx
	mov bx, 7
	div bx
	inc ax
fin12a:
	mov semana,ax
	cmp modo,2;confirma si el usuario ha pedido un día en específico
	je imprimir
	dec semana
	jmp paint3
imprimir:
	...
	imprimirString dx
	getposicion;obtiene la posición del cursor
	push b
...
imprimirNumeroDia:
	...
	imprimirString dx
	getposicion;obtiene la posición del cursor
	inc columna;aumenta la posición de la columna
...
imprimirRestantes:
	...
	getposicion;obtiene la posición del cursor
	inc columna;aumenta la posición de la columna
	mov ax, [n];resta el total de días con el numero de día actual, no confundir con fecha
	mov bx, [cant]
	sub ax, bx
...
finalizar2:
	terminar
;********************************************************************
;imprime las fechas del mes para su correspondiente día de la semana*
;********************************************************************
paint4:
	cmp d,5;viernes
	mov columna,56
...
paint3:
	...
	mov ax, ndias;confirma el limite de fecha según el mes
	inc ax
	cmp q, ax
	je setLimite3
...
;***************************************
;Limite según fila, similar a setLimite*
;***************************************
setLimite3:
	cmp fila,14
	mov limite2,278
...
;****************************************
;Limite según fila, similar a setLimite2*
;****************************************
setLimite4:
	cmp fila,14
	mov limite,246
